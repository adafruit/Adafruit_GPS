...
Adafruit_GPS Boat(&GPSSerial);    //*****build() example*****
#endif
...
#ifdef NMEA_EXTENSIONS
    char latestBoat[200] = "";                    //*****build() example*****
    updateBoat();                                 //*****build() example***** create some test data in Boat
    Boat.build(latestBoat,"GN","RMC");            //*****build() example***** make a sentence from Boat data
    Serial.print("\nbuild() test output -->");    //*****build() example*****
    Serial.print(latestBoat);                     //*****build() example*****
    GPS.resetSentTime();                          //*****build() example***** make timing look like it came in on GPS
    GPS.parse(latestBoat);                        //*****build() example***** parse the test data and store in GPS
#endif
  }
}

#ifdef NMEA_EXTENSIONS
void updateBoat(){  //*****build() example***** fill up the boat values with some test data to use in build()
